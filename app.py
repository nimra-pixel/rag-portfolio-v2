...
st.set_page_config(
    page_title="AI/ML Knowledge Base v2 — No API Key",
    page_icon="🧠",
    layout="wide",
...
KNOWLEDGE_BASE = [
    ...
    {"id": 1, "topic": "Retrieval-Augmented Generation (RAG)", "content": "Retrieval-Augmented Generation (RAG), introduced by Lewis et al. (2020), combines parametric knowledge stored in model weights with non-parametric knowledge retrieved from an external corpus at inference time. The pipeline has two phases: retrieval and generation. During retrieval, a dense vector search finds the top-k most relevant documents for a query using embedding similarity. During generation, those documents are prepended to the prompt as context, grounding the model response. RAG solves two key problems: knowledge cutoffs and hallucination. Hybrid RAG combines dense retrieval (vector search) with sparse retrieval (BM25 keyword search) and uses Reciprocal Rank Fusion to merge results. Production RAG systems measure quality with RAGAS metrics: faithfulness, answer relevancy, and context recall."},
    {"id": 2, "topic": "Large Language Models (LLMs)", "content": "Large Language Models are neural networks trained on massive text corpora using the self-supervised objective of next-token prediction. GPT-3 (175B parameters) demonstrated that scale alone could produce emergent capabilities including few-shot learning. Modern LLMs like GPT-4, Claude, Gemini, and Llama are trained in stages: pre-training on internet-scale text, supervised fine-tuning (SFT) on instruction-following examples, and RLHF to align outputs with human preferences. Key concepts include context window (maximum tokens the model can process), temperature (controls output randomness), and tokenisation. Inference efficiency techniques include KV-caching, flash attention, quantisation (INT8/INT4), and speculative decoding."},
    {"id": 3, "topic": "Fine-Tuning & LoRA", "content": "Fine-tuning adapts a pre-trained model to a specific task by continuing training on a smaller task-specific dataset. Full fine-tuning updates all model weights and is computationally expensive. LoRA (Low-Rank Adaptation) is the most widely used PEFT technique: it freezes pre-trained weights and injects trainable low-rank decomposition matrices into each transformer layer. Only these small matrices are trained, reducing trainable parameters by 10000x while achieving comparable performance to full fine-tuning. QLoRA extends LoRA with 4-bit quantisation of the base model, enabling fine-tuning of 70B parameter models on a single GPU. DPO (Direct Preference Optimisation) fine-tunes models on preference pairs without a separate reward model."},
    {"id": 4, "topic": "Embeddings & Vector Search", "content": "Embeddings are dense vector representations of text that capture semantic meaning. Similar meanings cluster together in vector space. Text embedding models like Sentence-BERT and E5 encode sentences into fixed-size vectors (typically 384-1536 dimensions). Cosine similarity measures the angle between two vectors, returning 1 for identical meaning and 0 for orthogonal. Vector databases (FAISS, Pinecone, Weaviate, Chroma) enable approximate nearest-neighbour search at scale using HNSW and IVF algorithms. FAISS (Facebook AI Similarity Search) is an open-source library optimised for billion-scale similarity search. The key tradeoff is recall vs latency: HNSW is fast but approximate; flat search is exact but slow."},
    {"id": 5, "topic": "Attention Mechanism", "content": "Attention allows a model to focus on relevant parts of the input when producing each output token. Scaled dot-product attention computes: Attention(Q,K,V) = softmax(QK^T / sqrt(d_k))V, where Q (queries), K (keys), and V (values) are linear projections of the input. Multi-head attention runs h parallel attention heads, each learning different relational patterns. Self-attention means Q, K, V all come from the same sequence. Causal masked self-attention prevents tokens from attending to future positions, essential for autoregressive generation. Flash Attention (Dao et al., 2022) reduces memory from O(N squared) to O(N) by tiling computations, enabling training on much longer sequences."},
    {"id": 6, "topic": "Evaluation Metrics for LLMs", "content": "Evaluating LLMs is hard because natural language has no single correct answer. Automatic metrics include BLEU (n-gram overlap), ROUGE (recall-oriented for summarisation), BERTScore (semantic similarity via embeddings), and perplexity. RAGAS is a framework specifically for RAG evaluation with three metrics: Faithfulness (does the answer use only retrieved context?), Answer Relevancy (does the answer address the question?), and Context Recall (did retrieval find the needed chunks?). LLM-as-judge uses a strong model to score outputs on rubrics. Human evaluation remains the gold standard but is expensive. MT-Bench and MMLU are popular benchmarks."},
    {"id": 7, "topic": "Prompt Engineering", "content": "Prompt engineering is the practice of designing inputs to LLMs to elicit desired outputs. Zero-shot prompting gives the model a task with no examples. Few-shot prompting includes 2-5 examples in the prompt. Chain-of-thought (CoT) prompting instructs the model to reason step by step before answering, improving performance on arithmetic and logical tasks. Structured output prompting uses JSON schemas or XML tags to enforce output format. System prompts set model behaviour and persona. Temperature controls randomness: 0 for deterministic factual tasks, 0.7-1.0 for creative tasks. Top-p nucleus sampling samples from tokens whose cumulative probability exceeds p."},
    {"id": 8, "topic": "RLHF & Alignment", "content": "RLHF aligns language models with human preferences through three stages. First, supervised fine-tuning (SFT) on high-quality human-written demonstrations. Second, reward model training where human labellers rank model outputs by preference. Third, RL optimisation using PPO (Proximal Policy Optimisation) to maximise reward scores, with a KL-divergence penalty to prevent the model drifting too far from the SFT baseline. InstructGPT showed that a 1.3B RLHF model was preferred over a 175B base model. Constitutional AI extends RLHF with an AI-generated critique-revision loop. DPO simplifies RLHF by directly optimising on preference pairs without a separate reward model."},
    {"id": 9, "topic": "Quantisation & Inference Optimisation", "content": "Quantisation reduces model size and speeds up inference by representing weights in lower precision. FP32 to FP16/BF16 halves memory with minimal quality loss. INT8 quantisation further halves memory. INT4/NF4 quantisation is used in QLoRA, compressing a 7B model to about 3.5GB. Post-training quantisation (PTQ) quantises a trained model without retraining. Beyond quantisation: KV-cache stores previously computed key-value pairs to avoid recomputation during autoregressive decoding. Speculative decoding uses a small draft model to propose tokens a larger model verifies in parallel, achieving 2-3x speedup. vLLM uses PagedAttention for efficient KV-cache management enabling high-throughput serving."},
]
...
.hero p  { color: #8b949e; margin: 0.5rem 0 0; font-size: 0.95rem; }
.hero .badge {
    display: inline-block;
    background: #1f4e2e; color: #3fb950;
...
.chunk-title { font-weight: 600; color: #58a6ff; font-size: 0.78rem; text-transform: uppercase; letter-spacing: .05em; margin-bottom: .4rem; }
...
def embed(texts: tuple) -> np.ndarray:
    r = requests.post(HF_EMBED, headers=hf_headers(),
        json={"inputs": list(texts), "options": {"wait_for_model": True}}, timeout=30)
...
def retrieve(query, top_k=3):
    ...
    qn = qe / (np.linalg.norm(qe)+1e-10)
    dn = de / (np.linalg.norm(de, axis=1, keepdims=True)+1e-10)
    vec = dn @ qn
...
def generate(query, chunks):
    ...
    prompt = (f"<|system|>\nYou are an expert AI/ML educator. Answer using ONLY the provided context. "
              ...
              f"If the answer is not in context, say 'This topic is not covered in my knowledge base.'\n</s>\n"
              f"<|user|>\nContext:\n{ctx}\n\nQuestion: {query}\n</s>\n<|assistant|>")
    try:
        ...
        pass
    # Fallback
    lines = ["**Retrieved directly from knowledge base** (LLM unavailable — showing top chunks):\n"]
    for c in chunks:
        sents = [s.strip() for s in c["content"].replace("\n"," ").split(".") if len(s.strip())>30]
        lines.append(f"**{c['topic']}** [SOURCE:{c['id']}]: {'. '.join(sents[:2])}.")
    return "\n\n".join(lines), "📚 Local fallback (retrieval only)"
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("""**Stack**  
    Streamlit · HuggingFace Inference API  
    all-MiniLM-L6-v2 · Zephyr-7B  
    BM25 + Vector · RRF fusion · NumPy
    """)
    st.caption("Built by Nimra · [v1 repo](https://github.com/nimra-pixel/rag-portfolio) · [v2 repo](https://github.com/nimra-pixel/rag-portfolio-v2)")
...
st.markdown("""
<div class="story-box">
  <strong>Why v2?</strong> — My first RAG app (v1) was built on the OpenAI API.
  ...
  So I rebuilt it from scratch using the <strong>HuggingFace free inference API</strong> —
  same hybrid retrieval architecture, same citation enforcement, zero cost to run.
  This is a real engineering decision you make in production: <em>how do you build for accessibility and cost?</em>
</div>
""", unsafe_allow_html=True)
...
if show_diff:
    ...
    st.markdown("""
    <div class="diff-row">
      <div class="diff-card v1"><h4>v1 — OpenAI</h4><p>text-embedding-3-small + GPT-4o-mini<br><br>❌ Requires paid API key<br>❌ Fails publicly with rate limit error<br>❌ Not accessible to everyone</p></div>
      <div class="diff-card v2"><h4>v2 — HuggingFace ✅</h4><p>all-MiniLM-L6-v2 + Zephyr-7B<br><br>✅ Completely free forever<br>✅ Works for any visitor<br>✅ Local fallback if LLM is slow</p></div>
      <div class="diff-card v2"><h4>Same core RAG ✅</h4><p>Hybrid BM25 + Vector search<br>Reciprocal Rank Fusion<br>Citation enforcement<br>Pipeline tracing</p></div>
      <div class="diff-card v2"><h4>New additions ✅</h4><p>v1 vs v2 diff panel<br>Generation source label<br>Graceful fallback mode<br>Leaner dependencies</p></div>
    </div>
    """, unsafe_allow_html=True)

# ── Metrics ───────────────────────────────────────────────────────────────────
c1, c2, c3, c4 = st.columns(4)
c1.metric("Topics indexed", "10")
c2.metric("Embeddings", "MiniLM-L6-v2 (free)")
c3.metric("Generation", "Zephyr-7B (free)")
...
if query:
    with st.spinner("Retrieving and generating..."):
        ...
        answer, source_label = generate(query, chunks)
    ...
    if show_pipeline:
        ...
        st.markdown(
            ...
            f"<span class='ps'>MiniLM vectors</span> → "
